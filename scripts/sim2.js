var dmz =
  ...
  , location = {}
  , animate = {}
  , plume = { list: [] }//  = { pos: dmz.vector.create(), handle: 0, radius: 0}
  , start = false
  , reportCount = 0
  , wind = dmz.vector.create(20, 0, 0)
//  Constants
  , InitialRadius = 330
...
updateReports = function (plume) {

   var r2 = (plume.radius - 40) * (plume.radius - 40)
     , keys
     ;
   ...
   keys.forEach(function (key) {
      ...
      if (!obj.inPlume && !obj.report) {

         if (r2 > plume.pos.subtract(obj.pos).magnitudeSquared()) {

            obj.inPlume = true;
            obj.start = dmz.time.getFrameTime();
...
/*
   ...
   keys.forEach(function (key) {
      ...
      location[key].inPlume = false;
      
   });
*/

   if (start && plume.source) {

      if (!plume.lastPlume || ((ftime - plume.lastPlume) > 3)) {

         handle = dmz.object.create(PlumeType);
         dmz.object.position(handle, undefined, plume.pos);
         dmz.object.scalar(handle, PlumeRadiusAttr, 10);
         dmz.object.activate(handle);
         plume.list.push(handle);
         plume.lastPlume = ftime;
      }

      plume.list.forEach (function (handle) {

         var pos = dmz.object.position(handle)
           , radius = dmz.object.scalar(handle, PlumeRadiusAttr);
           ;

           pos = pos.add(wind.multiply(time));
           radius += PlumeRate * time;

           dmz.object.position(handle, undefined, pos)
           dmz.object.scalar(handle, PlumeRadiusAttr, radius);

           updateReports({pos: pos, radius: radius});
      });
      ...
      while (!done) {
         ...
         if (plume.list.length === 0) { done = true; }
         else {

            radius = dmz.object.scalar(plume.list[0], PlumeRadiusAttr);
            if (radius > MaxRadius) {

               dmz.object.destroy(plume.list[0]);
               plume.list.shift();
            }
            else { done = true; }
         }
   ...
   keys.forEach(function (key) {
      ...
      if (obj.inPlume) {
         ...
         if (population) {
            ...
            casualties = Math.floor(((ftime - obj.start) / 200) * population);
            if (casualties > 100) { casualties = 100; }
         }
...
dmz.object.flag.observe(self, "Plume_Source", function (handle, attr, value) {

self.log.error("Source:", handle);
   if (value) {

      plume.source = handle;
      plume.pos = dmz.object.position(handle);
      plume.lastPlume = undefined;
...
dmz.object.vector.observe(self, "Env_Wind_Direction", function (handle, attr, value) {
